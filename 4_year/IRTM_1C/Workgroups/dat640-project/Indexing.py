...
def download_dataset(filename: str, force: bool = False) -> None:
    ...
    Args:
        ...
        force (optional): Downloads a file and overwrites if already exists.
            Defaults to False.
    """
    filepath = os.path.expanduser(f"~/.ir_datasets/msmarco-document/{filename}")
    if not force and os.path.isfile(filepath):
        return

    response = requests.get(f"https://msmarco.blob.core.windows.net/msmarcoranking/{filename}")
    if response.ok:
    ...
    print(next(ir_datasets.load("msmarco-document/train/scoreddocs").docs_iter()))
...
def index_collection(
    collection: str = "msmarco-document/train",
    filename: str = "inverted_index.sqlite",
    ...
    with InvertedIndex(filename, new=True) as index:
        for i, scoreddoc in enumerate(dataset.scoreddocs_iter()):
            if i % (num_documents // 10) == 0:
...
if __name__ == "__main__":
    ...
    download_dataset("msmarco-doctrain-top100.gz")
    #print("Indexing")
    #index_collection()  # total 100 docs