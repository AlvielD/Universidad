...
# Note from the students: We are modifying this script to adapt it to our program.
...
def record_video(process):
    ...
    while(nRet == ueye.IS_SUCCESS):
        ...
        if process:
            ...
            circles = cv2.HoughCircles(frame, cv2.HOUGH_GRADIENT, dp=1.2, minDist=20, param1=60, param2=40, minRadius=10, maxRadius=50)
            ...
            if circles is not None:
                ...
                # TODO print number of circles on a status bar?
        ...
        # TODO print frames on GUI and not on cv2.imshow() screen
        """image = qimage2ndarray.array2qimage(frame)
        app.pixmap = QPixmap.fromImage(image)
        app.scene.removeItem(self.curItem)
        app.curItem = QGraphicsPixmapItem(self.pixmap)
        app.scene.addItem(self.curItem)
        """

        # TODO use a stop video method?
