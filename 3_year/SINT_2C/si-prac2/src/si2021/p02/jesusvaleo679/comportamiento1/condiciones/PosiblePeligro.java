...
public class PosiblePeligro implements Condicion {
	...
	public boolean seCumple(StateObservation so) {
		...
		for (Observation o : proyectiles) {
			if ((o.position.x+FACTOR >= zona[0].x && o.position.x-FACTOR <= zona[0].x + 3/2*so.getBlockSize())
					|| (o.position.x+FACTOR >= zona[1].x && o.position.x-FACTOR <= zona[1].x + 3/2*so.getBlockSize())) {
				return true;
			}
		}
