...
public class PosiblePeligro extends Condicion {
	...
	public boolean seCumple(StateObservation so) {
		...
		Vector2d iniZona = new Vector2d(posJugador.x - so.getBlockSize(), posJugador.y - 1/2*so.getBlockSize());
		Vector2d finZona = new Vector2d(posJugador.x + 2*so.getBlockSize(), posJugador.y + 3/2*so.getBlockSize());
