...
public class NoPeligroIzquierda extends Condicion {
	...
	public boolean seCumple(StateObservation so) {
		...
		Vector2d iniZona = new Vector2d(posJugador.x + so.getBlockSize(), posJugador.y + so.getBlockSize());
		Vector2d finZona = new Vector2d(posJugador.x + so.getBlockSize(), posJugador.y + 2*so.getBlockSize());
